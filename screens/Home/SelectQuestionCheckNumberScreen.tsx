...
const SelectQuestionCheckNumberScreen: React.FC = () => {
  const navigation = useNavigation();
  const route =
    useRoute<RouteProp<HomeStackType, 'SelectQuestionCheckNumberScreen'>>();
  ...
  const onPressNext = () => {
    ...
    if (firstNumber?.length && lastNumber?.length) {
      navigation.navigate('WriteAnswerCheckScreen', {
  ...
  return (
    <SafeAreaView style={styles.container}>
      <Header title="답 확인" onPress={() => navigation.goBack()} />
...
export default SelectQuestionCheckNumberScreen;
